...
fn test_rbac_after_emergency_withdraw_can_unpause_and_reuse() {
    ...
    // Verify contract can be reused (balance is 0 now, so we need to mint more to lock)
    let token_admin_client = token::StellarAssetClient::new(&env, &token_client.address);
    token_admin_client.mint(&contract_client.address, &200);
    contract_client.lock_program_funds(&200);
    
    // Total balance should be 200 (the newly minted and locked amount)
